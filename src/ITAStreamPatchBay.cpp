...
ITAStreamPatchbay::ITAStreamPatchbay( const double dSamplerate, const int iBlockLength, const int iFadelength, const int iGainAdaption )
...
void ITAStreamPatchbay::SetInputDatasource( const int iInput, ITADatasource* pdsDatasource )
{
	...
	if( pdsDatasource != NULL )
	{
		...
		int iChannels = pdsDatasource->GetNumberOfChannels( );
		if( iChannels != m_vpInputs[iInput]->iChannels )
...
void ITAStreamPatchbay::ConnectChannels( const int iInput, const int iInputChannel, const int iOutput, const int iOutputChannel )
{
	// Delegate with default gain
	ConnectChannels( iInput, iInputChannel, iOutput, iOutputChannel, 1.0f );
}

void ITAStreamPatchbay::ConnectChannels( const int iInput, const int iInputChannel, const int iOutput, const int iOutputChannel, const double dGain )
{
	if( iInput >= GetNumInputs( ) )
...
double ITAStreamPatchbay::GetConnectionGain( const int iInput, const int iInputChannel, const int iOutput, const int iOutputChannel ) const
{
	if( iInput >= GetNumInputs( ) )
		ITA_EXCEPT1( INVALID_PARAMETER, "Input index out of range (in conn channel)" );
	if( iInputChannel >= m_vpInputs[iInput]->iChannels )
...
void ITAStreamPatchbay::SetConnectionGain( const int iInput, const int iInputChannel, const int iOutput, const int iOutputChannel, const double dGain )
{
	if( iInput >= GetNumInputs( ) )
		ITA_EXCEPT1( INVALID_PARAMETER, "Input index out of range (in conn channel)" );
	if( iInputChannel >= m_vpInputs[iInput]->iChannels )
...
void ITAStreamPatchbay::HandlePreGetBlockPointer( ITADatasourceRealization*, unsigned int )
{
	// Do nothing ...
}
	...
	for( int i = 0; i < iNumOutputs; i++ )
	{
		...
		for( int j = 0; j < GetOutputNumChannels( i ); j++ )
		{
			...
			for( std::set<Connection, CompareConnection>::iterator it = dest.begin( ); it != dest.end( ); ++it )
			{
				...
				float fCurrentGain = pOutput->fCurrentGain * pConnectedInChannel->fCurrentGain * it->fCurrentGain;
				float fNewGain     = pOutput->fNewGain * pConnectedInChannel->fNewGain * it->fNewGain;
