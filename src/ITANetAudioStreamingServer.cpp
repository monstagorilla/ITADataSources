...
bool CITANetAudioStreamingServer::LoopBody( )
{
	...
	if( iEstimatedClientRingBufferTargetLatencyFreeSamples >= m_iSendingBlockLength )
	{
		...
		// Besser w�re vermutlich, gleich alle samples zu senden und nicht nur einen Block nach dem anderen
...
#ifdef NET_AUDIO_SHOW_TRAFFIC
		vstr::out( ) << "[ITANetAudioStreamingServer] Transmitted " << m_iSendingBlockLength << " samples for " << m_pInputStream->GetNumberOfChannels( ) << " channels"
		             << std::endl;
#endif
