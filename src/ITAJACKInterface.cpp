...
{
public:
	ITAJackSource( int iChannels, double dSamplerate, int iBufferSize ) : ITADatasourceRealization( (unsigned int)iChannels, dSamplerate, (unsigned int)iBufferSize, 16 )
	{
	}
};


ITAJACKInterface::ITAJACKInterface( int blockSize ) : m_iBufferSize( blockSize )
{
	m_jackClient = NULL;
...
ITAJACKInterface::ITA_JACK_ERRORCODE ITAJACKInterface::Initialize( const std::string& clientName )
{
	...
	while( out_ports[m_iNumOutputChannels] )
		m_iNumOutputChannels++;
	while( in_ports[m_iNumInputChannels] )
		m_iNumInputChannels++;
