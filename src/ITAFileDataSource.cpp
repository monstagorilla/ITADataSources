...
ITAFileDatasource::ITAFileDatasource( std::string sFilename, unsigned int uiBlocklength, bool bLoopMode ) : ITABufferDatasource( )
{
	...
	if( m_uiFileCapacity > 0 )
	{
		...
		if( m_uiFileCapacity % uiBlocklength )
			k++;
...
{
	// Puffer freigeben
	for( unsigned int i = 0; i < vpfData.size( ); i++ )
		delete[] vpfData[i];
}

unsigned int ITAFileDatasource::GetFileCapacity( ) const
{
	return m_uiFileCapacity;
}
std::string ITAFileDatasource::GetFileName( ) const
{
	return m_sFilename;
}
