...
{
public:
	MySource( ITADatasource* pSource, long lChannels, long lBuffersize ) : ITADatasourceRealization( (unsigned int)lChannels, (unsigned int)lBuffersize )
	{
	...
	const float* GetBlockPointer( unsigned int uiChannel, bool bWaitForData = true )
	{
		if( bFirst )
		{
			for( unsigned int i = 0; i < m_uiChannels; i++ )
				memcpy( GetWritePointer( i ), pSource->GetBlockPointer( i, bWaitForData ), m_uiBlocklength * sizeof( float ) );
